...
$$
% FURTHER REDUCE THE ABOVE; SOLVE FOR NABLA
...
$$
\nabla_X \psi = X \psi + (2\pi i)\omega(X)\psi = X(\psi), % CHECK THIS
$$
where $\omega$ is the pullback so that
$$
\omega(X)\psi = \omega(X,X_\psi). % CHECK THIS TOO
$$
...
\end{thrm}
In other words, the prequantum line bundle is unique when $M$ is simply connected\footnote{Proofs may be found in \textbf{[Sniatycki]}, pp. 53-56, and \textbf{[Woodhouse]}, p. 160.}. % reprint the proofs here
...
%\begin{thrm}
%\emph{To be completed.} % classification of hermitian metrics on complex line bundles
%\end{thrm}

%Further, as implied earlier, we have a morphism of Lie algebras between the classical and quantum observables:
%\begin{thrm}
%\emph{To be completed.} % morphism of lie algebras between C^\infty(M) and Op(\mathcal{H}^{preq})?
%\end{thrm}
% ============================================================
