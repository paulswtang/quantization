...
In this chapter, we will review some basic and relevant results on the mathematical background behind the geometric quantization process --- mainly, the theories of the categories $\mathbf{Symp}$ of symplectic manifolds and symplectomorphisms/canonical relations, and $\mathbf{Hilb}$ of Hilbert spaces and unitary maps\footnote{Actually, it's not entirely clear-cut that these are the ``best'' morphisms for these respective categories when it comes to the problem of quantization --- see the chapter on structural properties of the geometric quantization process, as well as \cite{weinstein}.}. We also outline the three main steps of the quantization process:
\begin{itemize}
\item \emph{Pre-quantization}, where we construct an associated \emph{prequantum Hilbert space} out of a given symplectic manifold (assuming a few nice (co-)homological conditions) by considering the space of sections of an associated complex line bundle; this process is a natural one, but unfortunately the resulting Hilbert space still contains some rather un-physical properties. This leads to the next step of the process, polarization.
\item \emph{Polarization}, where we ``cut down'' the number of allowable states by half in order to enforce a physicality condition on the states of the quantum system; this is done by the choice of a \emph{polarization}, a kind of Lagrangian distribution on the tangent bundle of the phase space.
